<div align="center">

# Raphael Teixeira

### Professor · Matemática Aplicada, Sistemas Dinâmicos, Identificação, Controle e Inteligência Computacional.

[![Website](https://img.shields.io/badge/site-raphateixeira.github.io-1f6feb?style=flat-square&logo=googlechrome&logoColor=white)](https://raphateixeira.github.io/)
[![Twitter](https://img.shields.io/badge/twitter-%40TxRaphael-1f6feb?style=flat-square&logo=x&logoColor=white)](https://twitter.com/@RaphaelTei66265)
[![Email](https://img.shields.io/badge/email-raphaelbt%40ufpa.br-1f6feb?style=flat-square&logo=gmail&logoColor=white)](mailto:raphaelbt@ufpa.br)
[![UFPA](https://img.shields.io/badge/UFPA-Tucuruí%2C%20Pará-1f6feb?style=flat-square&logo=googlescholar&logoColor=white)](https://www.ufpa.br/)

</div>

Professor na **Universidade Federal do Pará (UFPA)**, campus de Tucuruí, atuando entre a sala de aula e a pesquisa aplicada em **sistemas dinâmicos, identificação de sistemas, controle e inteligência computacional**. Este repositório reúne notas de aula, laboratórios virtuais e projetos de orientação que também vivem no meu [site](https://raphateixeira.github.io/).

---

### 🔭 Linhas de pesquisa

- **Controle de sistemas dinâmicos** — clássico, espaço de estados, digital, MPC e controle orientado a dados (*data-driven control*)
- **Identificação de sistemas** — modelagem experimental de processos (térmicos, hidrológicos, eletromecânicos)
- **Inteligência computacional aplicada** — redes neurais, aprendizagem de máquina e modelos generativos para diagnóstico e controle
- **Eletrônica de potência** — conversores estáticos (Buck, VSC/VSI) e sistemas conectados à rede (DFIG)
- **Manutenção preditiva** — diagnóstico de transformadores de potência via análise de gases dissolvidos (DGA)

### 📚 Disciplinas & notas de aula

| Disciplina | Conteúdo |
|---|---|
| [ControleClassico](https://github.com/raphateixeira/ControleClassico) | Sistemas de Controle I — controle clássico contínuo, função de transferência |
| [ControleEstados](https://github.com/raphateixeira/ControleEstados) | Controle por espaço de estados |
| [ControleDigital](https://github.com/raphateixeira/ControleDigital) | Sistemas de controle digital (Ogata, Aguirre) |
| [MetodosNumericos](https://github.com/raphateixeira/MetodosNumericos) | Fundamentos matemáticos e algoritmos numéricos |
| [AlgebraLinear](https://github.com/raphateixeira/AlgebraLinear) | Álgebra Linear (UFPA) |
| [TEControle](https://github.com/raphateixeira/TEControle) | Tópicos Especiais em Controle e Aprendizagem de Máquina |

### ⚡ Controle & eletrônica de potência

- **[ControleBuck](https://github.com/raphateixeira/ControleBuck)** — controle de conversor Buck
- **[ControleDFIG](https://github.com/raphateixeira/ControleDFIG)** — controle de gerador de indução duplamente alimentado (aerogeradores)
- **[IC-DataDrive](https://github.com/raphateixeira/IC-DataDrive)** — iniciação científica em controle orientado a dados

### 🧪 Identificação de sistemas

- **[Identificacao](https://github.com/raphateixeira/Identificacao)** — métodos de identificação de sistemas dinâmicos (ajuste de reta, PRBS, ARX, modelo LIT)

### ⚙️ Protótipos, bancadas & laboratórios

- **[LabVirtual](https://github.com/raphateixeira/LabVirtual)** — laboratório virtual para modelagem e controle de sistemas dinâmicos
- **[Projeto-Aeropendulo](https://github.com/raphateixeira/Projeto-Aeropendulo)** — projeto do aeropêndulo
- **[FiltroSK](https://github.com/raphateixeira/FiltroSK)** — identificação e controle aplicados a filtro Sallen-Key
- **[CircuitosCA](https://github.com/raphateixeira/CircuitosCA)** — notas de circuitos de corrente alternada

### 🩺 Diagnóstico inteligente de equipamentos

- **[AnaliseDGA](https://github.com/raphateixeira/AnaliseDGA)** — sistema de diagnóstico inteligente de transformadores de potência (ANN, KNN, DUVAL), com o conjunto de dados de gases dissolvidos em óleo (DGA) usado no treinamento

### 🤖 Machine Learning, Deep Learning & leituras comentadas

- **[DeepLearning](https://github.com/raphateixeira/DeepLearning)** — notas e notebooks de aprendizagem de máquina e deep learning aplicada
- **[Manim](https://github.com/raphateixeira/Manim)** — coleção de animações matemáticas/didáticas

### 🎓 Orientações — Mestrado & TCC

- **[IdentificacaoJusante](https://github.com/raphateixeira/IdentificacaoJusante)** — modelagem da dinâmica hidrológica para previsão do nível de jusante da UHE Tucuruí
- **[MScOseias](https://github.com/raphateixeira/MScOseias)** — controle MPC-FSC de conversor VSC trifásico e gêmeo digital para laboratório virtual
- **[MScRaphael](https://github.com/raphateixeira/MScRaphael)** — dissertação de mestrado
- **[TCC-Diego](https://github.com/raphateixeira/TCC-Diego)** — trabalho de conclusão de curso

### 🧰 Templates & ferramentas

- **[TikZ](https://github.com/raphateixeira/TikZ)** — figuras em TikZ para materiais didáticos
- **[TemplateNotas](https://github.com/raphateixeira/TemplateNotas)** — template de notas de aula em Quarto

### 🛠️ Stack

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![MATLAB](https://img.shields.io/badge/-MATLAB-0076A8?style=flat-square&logo=mathworks&logoColor=white)
![Jupyter](https://img.shields.io/badge/-Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white)
![LaTeX](https://img.shields.io/badge/-LaTeX-008080?style=flat-square&logo=latex&logoColor=white)
![PyTorch](https://img.shields.io/badge/-PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![Quarto](https://img.shields.io/badge/-Quarto-39729E?style=flat-square&logo=quarto&logoColor=white)

---

<div align="center">

![GitHub stats](https://github-readme-stats.vercel.app/api?username=raphateixeira&show_icons=true&hide_title=true&hide_border=true&theme=default)
![Top langs](https://github-readme-stats.vercel.app/api/top-langs/?username=raphateixeira&layout=compact&hide_border=true&theme=default)

</div>
